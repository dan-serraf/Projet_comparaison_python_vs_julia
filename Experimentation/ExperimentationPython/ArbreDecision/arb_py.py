...
sys.path.append('../../../Biblioteque/BibliotequePython/Utilitaire')
...
for train_x, test_x, train_y, test_y in tqdm(list_data) :
    ...
    debut = time.time()
    arb =  cls.ClassifierArbreDecision(profondeur)
    attributes = np.array([i for i in range(train_x.shape[1]) ])
    type_attribute = np.array(['float' for _ in range(train_x.shape[1]) ])
    attribut_valeurs = ut.dict_continus_discret(attributes,type_attribute)
    arb = arb.fit(train_x,train_y,attributes,type_attribute,attribut_valeurs)
    acc = arb.accuracy(test_x, test_y)
    fin = time.time()
